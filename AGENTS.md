# VITAL-APPOINTMENT Agent Rules

This repository is part of the Bernardino General Hospital workspace. Use this file as the root entry point for agent behavior in `VITAL-APPOINTMENT`.

## 1. Read These First

Before changing code or docs in this repo, read:

1. `.agents/rules/appointment-context-documents.md`
2. `.agents/rules/appointment-gateway-auth-contract.md` when the task touches auth, routing, trusted headers, roles, or internal service calls
3. `README.md`
4. `docs/README.md`
...
- Current business controllers are mostly placeholders plus the default root hello endpoint.
...
- The repo now includes a Dockerfile plus a repo-local compose stack for the app and a PostgreSQL sidecar.
