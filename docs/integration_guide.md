...
This document describes how `VITAL-APPOINTMENT` should integrate with `BGH_API_GATEWAY` and `UHSE_AUTH`.
...
```text
browser -> BGH_API_GATEWAY -> /vital/appointments* | /vital/payments* | /vital/notifications* -> VITAL-APPOINTMENT
```
...
- do not invent a separate public system slug such as `/appointment/*` unless the Gateway contract is intentionally changed and documented

Current repo reality:

- the checked-in workspace Gateway routes `/vital/appointments*`, `/vital/payments*`, and `/vital/notifications*` to `VITAL-APPOINTMENT`
- Auth treats this repo as part of the shared `vital` system and may union its local roles with `VITAL-TELEMEDICINE` under one `vital` claim
...
```http
X-Internal-Service: vital-appointment
...
- future audit or notification events
