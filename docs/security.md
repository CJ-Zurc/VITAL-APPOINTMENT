...
This document records the trust and integration rules `VITAL-APPOINTMENT` should follow as it moves behind the BGH platform boundary.
...
5. `VITAL-APPOINTMENT` validates `X-Gateway-Secret`
6. `VITAL-APPOINTMENT` authorizes business actions from trusted headers and local rules
...
- empty `appointment` controller
- empty `payment` controller

Those defaults are not yet the intended final VITAL appointment security model.
