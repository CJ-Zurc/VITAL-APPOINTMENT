---
trigger: always_on
description: Commit message rules for the VITAL-APPOINTMENT repository, aligned with the shared BGH developer guidelines.
---
...
- Use a scope that matches the area changed, such as `appointment`, `payment`, `docs`, `auth`, or `gateway`.
- When referring to files, use paths relative to the repo root such as `docs/integration_guide.md` or `src/appointment/appointment.controller.ts`.
