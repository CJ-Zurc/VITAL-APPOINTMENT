---
trigger: always_on
description: Primary onboarding and documentation map for the VITAL-APPOINTMENT repository.
---

# VITAL-APPOINTMENT Context Documents
...
5. `docs/security.md`
6. `.agents/rules/appointment-gateway-auth-contract.md`
...
- current checked-in HTTP behavior is limited to the default root response, an Auth-facing empty-role endpoint, and placeholder business controllers
- the repo now includes a Dockerfile and repo-local compose stack for the app plus PostgreSQL
- the checked-in workspace Gateway currently maps `/vital/appointments*`, `/vital/payments*`, and `/vital/notifications*` to this repo's domain boundary
