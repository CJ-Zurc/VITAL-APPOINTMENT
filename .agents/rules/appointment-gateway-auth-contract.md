---
trigger: always_on
description: Non-negotiable Gateway and Auth integration rules for VITAL-APPOINTMENT.
---

# VITAL-APPOINTMENT Gateway and Auth Contract
...
- The current checked-in Gateway path ownership for this repo is `/vital/appointments*`, `/vital/payments*`, and `/vital/notifications*`.
...
- Preserve it in logs, future audit events, future notifications, and any direct Auth internal calls.
...
- prefer `X-Internal-Service: vital-appointment`
...
- Auth may aggregate roles from this repo together with `VITAL-TELEMEDICINE` under the shared `vital` claim
